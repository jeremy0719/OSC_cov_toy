...
void usage(int nInputs)
{
  ...
  printf("Usage: ./DrawChiPicture list_of_files list_of_legends_files\n");
}
...
int main(int argc, char** argv)
{
  ...
  // Set contour level
  // 95% =5.99, 3 sigma=11.83
  double contourSig95[1] = {5.99}; // 95% C.L
  double contourSig3S[1] = {11.83}; //3Sigma
  ...
  for (unsigned int i = 0; i < inputFiles.size(); i++) {
    TH2F* temp;
    temp = (TH2F*)inputFiles[i]->Get("CumulativeChiSquareMap");
    
    int x,y,z;
    temp->GetMinimumBin(x,y,z);
    double minValue=temp->GetBinContent(x,y);
    
    for (int j = 1; j <= temp->GetXaxis()->GetNbins(); j++) {
      for (int k = 1; k <=temp ->GetYaxis()->GetNbins(); k++) {
        double tempBinValue=temp->GetBinContent(j,k)-minValue;
        temp->SetBinContent(j,k,tempBinValue);
      }
    }
    ...
    temp->SetLineWidth(2);
    temp->SetContour(1, contourSig3S);
  ...
  leg->SetHeader("#font[22]{Sensitivity: 3 #sigma CL}");
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  leg->SetBorderSize(0);
  ...
  TGraph *DANSSG=new TGraph();
  makeTXTTGraph("inputs/SensExcCurves/DANSS_Syst_95.txt",DANSSG);
  DANSSG->SetLineColor(kTeal);
  DANSSG->SetLineWidth(2);
  DANSSG->SetLineStyle(2);
  
  TGraph *NEOSG=new TGraph();
  makeTXTTGraph("inputs/SensExcCurves/Neos_sensitivity_95CL.txt",NEOSG);
  NEOSG->SetLineColor(kPink-2);
  NEOSG->SetLineWidth(2);
  NEOSG->SetLineStyle(2);
  
  TGraph *SOLIDG=new TGraph();
  makeTXTTGraph("inputs/SensExcCurves/Solid_Sensitivity_150days_CL95.txt",SOLIDG);
  SOLIDG->SetLineColor(kGreen-2);
  SOLIDG->SetLineWidth(2);
  SOLIDG->SetLineStyle(2);
  
  TGraph *STEREOG=new TGraph();
  makeTXTTGraph("inputs/SensExcCurves/Stereo_NormShape95_300days.txt",STEREOG);
  STEREOG->SetLineColor(kBlue-2);
  STEREOG->SetLineWidth(2);
  STEREOG->SetLineStyle(2);
...
//  leg->AddEntry(DANSSG,"DANSS Exclusion 95% CL - 170 days of data","l");
//  leg->AddEntry(NEOSG,"NEOS Exclusion 95% CL - 180 days of data","l");
//  leg->AddEntry(SOLIDG,"SOLID 95% CL - 150 days","l");
//  leg->AddEntry(STEREOG,"STEREO 95% CL - 300 days","l");

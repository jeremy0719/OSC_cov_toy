...
void CreateSignalEScaleSegmentMatrix(int nToys = 100) {
  ...
  TH2F* SegvsENull = (TH2F*)gFile->Get("SegvsENull101");
  TH2F* LvsENull = (TH2F*)gFile->Get("LvsENull101");
  TH2F* LToSeg = (TH2F*)gFile->Get("PositionToSegmentFid101");
  
  // Create Output File
  TString outputName(inputName);
  outputName.ReplaceAll("Set", "Cov_Sys_EScaleSegment");
  std::cout << "Output file is: " << outputName.Data()<<std::endl;

...
void usage(int arguments)
{
  ...
  printf("Usage:\n");
  printf("$ ./runMultipleMinimization ./macro.mac\n");
}
...
int main(int argc,char** argv)
{
  ...
  // Will be overwritten if defined in macro file
  ...
  //oscillation.WriteHistograms(*outputOscFile);//Write histograms to the outputOscFile
  
  TFile* outputMinimizationFile = new TFile(outputMinimizationFileName,"RECREATE"); //
  
  // Minimization object, based on what we need to do, we can either make it a TSenstivity object or TOscillatioFitting object
  ...
  for(int i=0;i<nToys;i++)
  {
    ...
    printf("Running toy %i, dt=%f seconds\n",i,dt);
    time(&prevTimer);
  }
